...
//        }
    ...
    public void transfer(byte[] imgByte){
        Message msg = mHandler.obtainMessage(Constants.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(Constants.TOAST, "開始傳照片");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
            ...
            while (mState == STATE_CONNECTED) {
                ...
                try{
                    ...
                    try {
                        ...
                        //mHandler.obtainMessage(Constants.MESSAGE_TRANS, baos).sendToTarget();
...
/*
        ...
        public void transfer(byte[] imgByre){

            byte[] buffer = new byte[990];
            int length;
            ByteArrayInputStream bais = new ByteArrayInputStream(imgByre);
            try{
                while((length= bais.read(buffer))!=-1) {
                    mmOutStream.write(buffer, 0, length);
                    Log.v("aaa","送出byte: "+length);
                }
            }
            catch (Exception e){}
            Message msg = mHandler.obtainMessage(Constants.MESSAGE_TOAST);
            Bundle bundle = new Bundle();
            bundle.putString(Constants.TOAST, "傳送完畢");
            msg.setData(bundle);
            mHandler.sendMessage(msg);
            //test
                ...
                synchronized (BluetoothChatService.this){
                    try{
                        while (mmInStream.available()>0){
                            Log.v("aaa","接收數據"+mmInStream.available());
                            bytes = mmInStream.read(buffer);
                        }
                    }
                    catch (Exception e){
